...
materials. The Streamlit interface is an enterprise-style learning
operations dashboard: it shows the recommended next action, operational
readiness gates, learning state, schedule, course structure, and latest
assessment evidence in one place.

The system was built to demonstrate the integration of six distinct AI
paradigms in a single coherent application.
...
The default LLM provider is `mock`, which gives deterministic,
context-aware responses so the demo runs without API access. It uses
the selected topic and retrieved chunk IDs, so the UI can be presented
offline without fake-looking citations. Switch to `anthropic` from the
sidebar once your key is configured.

The included academic demo data lives in:

```
data/demo_academic/
├── syllabus.txt
├── linear_algebra_material.txt
└── regression_material.txt
```

### Dashboard workflow

For a clean final presentation, use the dashboard in this order:

1. Open `http://localhost:8501`.
2. Click **Reset workspace** in the sidebar.
3. **Setup**: use the included syllabus and click **Extract knowledge graph**.
4. **Weekly**: ingest the default `linear_algebra` material.
5. **Session**: run the `linear_algebra` study session. The BKT estimate should move above the mastery threshold.
6. **Planner**: confirm the recommended next topic is now `regression`.
7. **Weekly**: ingest `regression` material before running a regression session.
8. **Disruption**: report a natural-language change such as `I was sick today and missed my planned regression study session.`
9. Return to **Planner** and show the updated schedule and readiness gates.

The dashboard has guardrails: it will not run a study session for a topic
that has no ingested RAG material, and it reconciles the schedule against
current mastery so already-mastered topics do not remain as the next
planned `INTRODUCE_NEW` session.

### Dashboard sections

| Section | Purpose |
|---|---|
| **Decision panel** | Shows the single recommended next action for the operator. |
| **Operational readiness** | Confirms whether KG, RAG evidence, BKT mastery, schedule, and assessment evidence are ready. |
| **Learning state** | Displays per-topic mastery and status (`Needs review`, `In progress`, `Mastered`). |
| **Schedule** | Shows the next planned session after rule-based feasibility and disruption reflow. |
| **Course structure** | Displays the verified knowledge graph topics and mastery state. |
| **Latest evidence** | Shows the latest quiz/BKT evidence generated by a study session. |
...
│   └── ui/                    Streamlit learning-operations dashboard
├── tests/                     unit + integration tests
├── evaluation/                three reproducible benchmarks + summary
├── scripts/                   sim_demo.py, smoke_test.py
└── data/                      ASSISTments data and academic demo fixtures
```
...
- **Streamlit** for the learning-operations dashboard
...
- The LLM model version is pinned in `config/default.yaml`.
- The default `mock` provider is deterministic and context-aware, so
  the Streamlit demo and CI can run offline. The academic demo fixtures
  are stored in `data/demo_academic/`.
